...
export function accept_bid(
  ...
  bidShares.owner = ONE_HUNDRED_PERCENT - bid.sell_on_share - bidShares.creator;
...
export function burn(token_id: string): void {
  ...
  for (let i = 0; i < bidders.size; i++) {
    let promise = ContractPromiseBatch.create(biddersValues[i].bidder).transfer(
      biddersValues[i].amount
    );
    env.promise_return(promise.id);
