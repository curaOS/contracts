...
import { CONTRACT_EXTRA, CONTRACT_METADATA, TOKEN_ROYALTY, TOKEN_METADATA } from '../utils/dummyData';
import { call_accept_bid, call_burn_design, call_init, call_mint, call_nft_transfer, call_remove_bid, call_set_bid, view_nft_supply_for_owner, view_nft_token, view_nft_tokens_for_owner } from '../utils/functions';
...
workspace.test(
    ...
    async (test, { contract, alice }) => {
        ...
        test.deepEqual(fetched.metadata.description, args.tokenMetadata.description)
...
workspace.test(
    ...
    async (test, { contract, john, alice }) => {
        ...
        const { result: fetched } = await view_nft_token(contract, { token_id: minted.id })
        test.deepEqual(fetched.owner_id, "")
        log(`✓  Alice can fetch updated token after burn\n`)

        const { result: fetched_alice_tokens } = await view_nft_tokens_for_owner(contract, { account_id: alice.accountId })
        test.assert(fetched_alice_tokens.length == 0)
        log(`✓  Alice doesn't own burned token anymore\n`)

        const { result: fetched_alice_supply } = await view_nft_supply_for_owner(contract, { account_id: alice.accountId })
...
workspace.test(
    ...
    async (test, { contract, john, alice }) => {
        ...
        const { result: fetched_john_tokens } = await view_nft_tokens_for_owner(contract, { account_id: john.accountId })
